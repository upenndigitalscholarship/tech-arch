...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      plotOutput("correlationPlot", width = "70%", height = "600px"),
      width = 15
    )
  )
)
...
server <- function(input, output) {
  ...
  output$correlationPlot <- renderPlot({
    ...
    custom_palette <- colorRampPalette(c("gray", "white"))(100)  # Gray for negative correlation
    custom_palette <- c(custom_palette, colorRampPalette(c("white", "blue"))(100))  # Blue for positive correlation
    ...
    corrplot(
      ...
      tl.col = "black",
      tl.srt = 30,   # Adjust text rotation angle
      tl.pos = "lt",
      mar = c(1, 1, 5, 1),   # Adjust the plot margins (left, right, top, bottom)
      col = custom_palette  # Use the custom color palette
    ) 
  })
}
